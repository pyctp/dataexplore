...
def BARSSK():
    if lastsig and (lastsig[-1][1] == 'SK' or lastsig[-1] == 'SPK'):
        if sigall[-1] == 'SK' or sigall[-1] == 'SPK':
            ret = 0
        else:
            ret = sigall[-1]
    else:
...
RSV2 = (C - pd.Series(LLV(L, 3))) / (pd.Series(HHV(H, 3)) - pd.Series(LLV(L, 3))) * 100
KK = sma(RSV2, 59, 1)
DD2 = sma(KK, 105, 1)
...
for i in range(len(bars)):
    print(i, bars.iloc[i].datetime, BARSBK(), BARSSK())
    ...
    if (CbMAA.iloc[i] and K.iloc[i] < 48 and ISUP.iloc[i] and (AA.iloc[i] or AA2.iloc[i]) and BARSSK() > 109) and FANSHOU == 0 and QBP:
        #     BP(SKVOL)
        signal = 'BP'
        dt = bars.iloc[i].datetime
        order_price = bars.close.iloc[i]
        lastsig.append([dt, signal, order_price])
        sigall.append('BP')
        sigcount = 0


    elif (CbMAA.iloc[i] and K.iloc[i] < 48 and ISUP.iloc[i] and (AA.iloc[i] or AA2.iloc[i]) and BARSSK() > 109) and FANSHOU == 0 and QBK:
        #     BK(SS)
        signal = 'BK'
        dt = bars.iloc[i].datetime
        order_price = bars.close.iloc[i]
    ...
    elif (CbMAA.iloc[i] and K.iloc[i] > 39 and ISDOWN.iloc[i] and (AA.iloc[i] or AA2.iloc[i]) and BARSBK() > 50) and QSP:
        #     SP(BKVOL)
        # print((CbMAA.iloc[i] and K.iloc[i] > 39 and ISDOWN.iloc[i] and (AA.iloc[i] or AA2.iloc[i]) and BARSBK() > 50) and QSP )
        signal = 'SP'
        dt = bars.iloc[i].datetime
        order_price = bars.close.iloc[i]
        lastsig.append([dt, signal, order_price])
...
lastsigpd = pd.Series(lastsig)
lastsigpd.to_json('lastsig.json')
