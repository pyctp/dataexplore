...
def BARSSK():
    if lastsig and (lastsig[-1] == 'SK' or lastsig[-1] == 'SPK'):
...
for i in range(len(bars)):
    ...
    if (CbMAA.iloc[i] and K.iloc[i] < 48 and ISUP.iloc[i] and (AA.iloc[i] or AA2.iloc[i]) and BARSSK() > 109) and FANSHOU == 0 and QBP:
        ...
        #     BP(SKVOL)
        signal = 'BP'
        dt = time_to_str(bars.iloc[i].datetime)
    ...
    elif CbMAA.iloc[i] and K.iloc[i] < 48 and ISUP.iloc[i] and (AA.iloc[i] or AA2.iloc[i]) and BARSSK() > 109 and FANSHOU == 1:
        #     BPK(SS)
        signal = 'BPK'
        dt = time_to_str(bars.iloc[i].datetime)
        order_price = bars.close.iloc[i]
        lastsig.append([bars.iloc[i].datetime, 'BPK', bars.close.iloc[i]])
        sigall.append('BP')
        sigcount = 0
